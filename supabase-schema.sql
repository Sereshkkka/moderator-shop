...
create table if not exists public.logs (
    ...
    modifier_id text null references public.users(id) on delete set null,
    company_id text not null references public.companies(id) on delete cascade,
...
insert into public.system_config (id, webhook_url, avatar_url_template)
values (true, '', 'https://skins.mcskill.net/?name=insert&mode=5&fx=size&fy=size')
on conflict (id) do nothing;

insert into public.companies (id, name, webhook_url)
values ('comp_initial', 'Главный Сервер', '')
on conflict (id) do nothing;

insert into public.roles (id, label, tier, color, perms) values
...
create index if not exists idx_codes_company on public.codes(company_id);
...
revoke all on public.user_company_access from anon, authenticated;
